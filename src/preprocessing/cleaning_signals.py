...
from src.preprocessing.cleaning_metadata import FilterBandConfig, NO_FILTER_LOW_CUT_HZ, NO_FILTER_HIGH_CUT_FRACTION_OF_NYQUIST, FILTER_BAND_FIXED_CORNERS
...
def apply_bandpass_filter(
    ...
    Uses forward-backward filtering (filtfilt) to match the acausal
    processing described for ITACA in Paolucci et al. (2011), avoiding
    the phase distortion of a standard causal filter.
    ...
    sampling_rate : float
        Sampling rate in Hz.
    order : int, optional
        Filter order, applied twice due to zero-phase filtering, for an
        effective order of 2 * order (default: 2, matching the
        second-order Butterworth filter used by ITACA).
    ...
    ------
    ValueError
        If low_cut >= high_cut, or if high_cut is not strictly below the
        Nyquist frequency (sampling_rate / 2).
    """
    nyquist = sampling_rate / 2
    if low_cut >= high_cut:
        raise ValueError(f"low_cut ({low_cut}) must be < high_cut ({high_cut}).")
    if high_cut >= nyquist:
        raise ValueError(
            f"high_cut ({high_cut} Hz) must be strictly below the Nyquist "
            f"frequency ({nyquist} Hz) for sampling_rate={sampling_rate} Hz."
        )
...
def apply_filter_band_to_signals(
    df_signals: pd.DataFrame,
    ...
    """
    Apply one of the four filter band sensitivity configurations to the
    signal amplitudes in df_signals, filtering each file independently.

    Parameters
    ----------
    df_signals : pd.DataFrame
        Long-format signal data with columns ['file', 'sample',
        signal_column], one row per sample.
    config : {'no_filter', 'itaca_baseline', 'narrow', 'wide'}
        Filter band configuration, using the same corner frequencies as
        set_filter_band().
    signal_column : str
        Name of the signal value column to filter in place.
    sampling_rate : float
        Sampling rate in Hz.
    get_station_from_filename : callable
        Function mapping a filename to a station code.
    get_component_from_filename : callable
        Function mapping a filename to a component code.
    df_meta_itaca_processed : pd.DataFrame, optional
        Processed ITACA metadata containing per-station
        LOW_CUT_FREQUENCY_HZ / HIGH_CUT_FREQUENCY_HZ. Required only when
        config='itaca_baseline'.
    join_keys : list of str, optional
        Columns used to match corner frequencies to files when
        config='itaca_baseline' (default: ['STATION_CODE', 'COMPONENT']).

    Returns
    -------
    pd.DataFrame
        Copy of df_signals with signal_column replaced by its filtered
        values. Sample indices and all other columns are unchanged.

    Raises
    ------
    ValueError
        If config='itaca_baseline' and df_meta_itaca_processed is not
        provided, or if a file's station/component has no matching
        corner frequencies.
    """
    if config == 'itaca_baseline' and df_meta_itaca_processed is None:
        raise ValueError(
    ...
    print(f"Applying '{config}' filter band to {len(file_list)} file(s)...")
    ...
    for file_name in file_list:
        ...
        if config == 'no_filter':
            ...
            corner_row = df_meta_itaca_processed[
                (df_meta_itaca_processed[join_keys[0]] == station)
                & (df_meta_itaca_processed[join_keys[1]] == component)
            ]
            if corner_row.empty:
                raise ValueError(
                    f"No ITACA baseline filter corners found for "
                    f"station='{station}', component='{component}'."
                )
            low_cut = corner_row['LOW_CUT_FREQUENCY_HZ'].iloc[0]
            high_cut = corner_row['HIGH_CUT_FREQUENCY_HZ'].iloc[0]
        else:
            raise ValueError(
        ...
        file_rows = df[df['file'] == file_name].sort_values('sample')
        original_signal = file_rows[signal_column].to_numpy()
        filtered_signal = apply_bandpass_filter(
            original_signal, low_cut, high_cut, sampling_rate
        )

        file_rows = file_rows.copy()
        file_rows[signal_column] = filtered_signal
        filtered_blocks.append(file_rows)

    df_filtered = pd.concat(filtered_blocks, ignore_index=True)
    print(f"Filtering complete: {len(file_list)} file(s) filtered.")
